...
void uart_SendBuffer(void) {
	char str[100];
	uint8_t len = sprintf(str, "head:%u", (unsigned int) ring_buffer.head);
	HAL_UART_Transmit(&huart2, (void*) str, len, 100);
	len = sprintf(str, "tail:%u", (unsigned int) ring_buffer.tail);
	HAL_UART_Transmit(&huart2, (void*) str, len, 100);
}
void uart_SendAdcVal(void) {
	char str[20];
	uint8_t len = sprintf(str, "!ADC=%u#", (unsigned int) fsm_value());
